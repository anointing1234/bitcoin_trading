...
class ForexPlanAdmin(UnfoldModelAdmin):
    list_display = ('name', 'percentage', 'duration', 'min_amount', 'max_amount')
    search_fields = ('name', 'duration')
    list_filter = ('percentage', 'duration')
...
class UsersInvestmentAdmin(UnfoldModelAdmin):
    list_display = ('user', 'uniq_id', 'title', 'profit', 'daily_income', 'total', 'ends_in', 'elapsed', 'status')
    search_fields = ('user__email', 'uniq_id', 'title')
